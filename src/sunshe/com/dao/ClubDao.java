...
public class ClubDao extends BaseDao {
	...
	public boolean deleteClub(String CLno) {
		...
		String sql2= "delete from clubscourses where clno=?";
	...
	//修改社团
		...
		//获得某社团内各个课程某年份的选课人数
				...
				//删除学生所选社团   new
				public boolean deleteStudentClub(String CLno, String CLabout) {
					// TODO Auto-generated method stub
					String sql = "DELETE FROM clubs WHERE clno=? AND clabout=?";
					try (Connection conn = dataSource.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
						pstmt.setString(1, CLno);
						pstmt.setString(2, CLabout);
						pstmt.executeUpdate();
						conn.close();
						return true;
					} catch (Exception se) {
